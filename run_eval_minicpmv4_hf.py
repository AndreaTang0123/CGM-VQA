"""
Batch evaluation of CGM-VQA using MiniCPM-V 4 (openbmb/MiniCPM-V-4) via HuggingFace.

Requirements:
    pip install transformers accelerate pillow torch torchvision einops timm

Results saved to: results/minicpmv4_hf_results.json

Usage:
    python3 run_eval_minicpmv4_hf.py
    python3 run_eval_minicpmv4_hf.py --batch-size 1 --model openbmb/MiniCPM-V-4
"""
...
RESULTS_FILE = RESULTS_DIR / "minicpmv4_hf_results.json"

DEFAULT_MODEL      = "openbmb/MiniCPM-V-4"
...
def load_model(model_id: str):
    ...
    model = AutoModel.from_pretrained(
        ...
        torch_dtype=torch.float16,   # float16 for V100 coverage
        device_map="auto",
...
def run_batch(model, tokenizer, batch: list[dict]) -> list[str]:
    """Run inference on a batch; return list of answer strings."""
    answers = []
    
    # Note: Using batch_size=1 sequentially, as MiniCPM-V .chat() processes one sample easily.
    for sample in batch:
        ...
        prompt = (
            ...
            f"Answer format: {sample['expected_answer_format']}\n"
            f"Answer:"
        )
        ...
        with torch.no_grad():
            res = model.chat(
                ...
                msgs=msgs,
                tokenizer=tokenizer,
                sampling=False, # Use greedy decoding for determinism
                max_new_tokens=MAX_NEW_TOKENS
            )
        
        # MiniCPM-V's chat returns string directly (sometimes with special tokens if skipped wrongly, 
        # but `.chat()` usually handles it).
        
        # Make sure things are safe if it's a tuple for some reason
...
def run_evaluation(model_id: str, batch_size: int):
    ...
    for batch_start in range(0, len(remaining), batch_size):
        ...
        t0 = time.time()
        try:
            answers = run_batch(model, tokenizer, batch)
            ...
            print(f"ERROR: {e}")
            errors += len(batch)
            # Break early upon first failure for debuggability
            break
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="MiniCPM-V 4 CGM-VQA Evaluation (HuggingFace + GPU)")
